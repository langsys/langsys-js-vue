...
- **`useServerMessage()` — render server message entries.** A Langsys server SDK attaches translatable entries — `{ template, params?, message, field?, code? }` — beside your framework's own error body, under a key you configure. `render(entry)` shows the template's translation, filled from its params, when the catalog has one, and the server's own `message` otherwise; it re-renders on locale and catalog changes. The decision is the base SDK's `renderServerMessage()`, re-exported with `resolveServerMessages(body, { key })`, which reads entries only from the key or resolver you give it — including an Inertia page's props — and never changes the body.

- **`data-ls-resolved` is honoured through `<Translate>` and `<Phrase>`.** Both hand the base SDK their host in the page, so text under an element marked as already rendered in the visitor's locale is not registered as new source. `useT()` is outside the marker by design: a `t()` argument is source text the app supplies.
