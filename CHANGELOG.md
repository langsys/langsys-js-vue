...
- **`LangsysApp` is now a `Proxy` over the base SDK singleton rather than a hand-written wrapper class.** It forwards every core member **by reference** and overrides exactly two — `init` and `setWriteGrant`, the only two that adapt Vue shapes. Non-breaking: `LangsysApp.getCountries()` and every other call keep working, and now resolve to the core's own function rather than a copy of it.
    ...
    **Open — destructuring regressed, and the shape is pending a fleet ruling.** Measured against the last published release: `const { getCountries } = LangsysApp; getCountries()` **works on 0.2.1** (the old wrapper delegated in its own body, so it never needed `this`) and **throws** on the proxy (`Cannot read properties of undefined (reading 'resolveLocale')`), because an unbound forward calls with `this === undefined`. A non-destructured call works on both. So the argument above — that binding would make a destructured method behave differently here than off the core singleton — describes a difference this package **already shipped**, which weakens it as a reason to keep the regression. Two shapes are on the table: bind forwarded members (restoring 0.2.1 behaviour, at the cost of `LangsysApp.foo === core.foo` identity), or keep unbound and carry this as breaking. Routed to the fleet reviewer; this entry will be updated with the ruling rather than left to a reader to discover.
