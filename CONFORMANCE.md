...
| Suite                       | **85 tests / 8 files**, all passing (includes the 5-test upstream precondition and the 34-test BIND-6 surface guard)                                                                      |
| Evidence grade of the suite | **`mock`** — node env, core doubled where it matters, zero network                                                                                                                        |
| Core consumed               | `langsys-js-typescript` working copy @ `974081b`, local `0.6.5` via a gitignored `node_modules` symlink — **not** the published `0.6.5`. Every probe count below is baselined at this SHA |
...
| **BIND-6** — wrap the narrowest surface possible                  | `provisional` | `LangsysApp` is a **`Proxy` over the core singleton**, forwarding every member by reference with exactly two overrides (`init`, `setWriteGrant` — the only two that adapt Vue shapes under accepted deviation A). The 14 pure pass-through wrappers are gone. This fixed a live defect, not a smell: the enumerating class had **already silently dropped five core methods** (`applyAuthorization`, `getUserLanguagePreferences`, `parseAcceptLanguageHeader`, `findBestLocaleMatch`, `resolveLocale`) with a green suite, because nothing referenced what was missing. All five are reachable again. Forwarded members are returned **unbound**, so identity holds (`LangsysApp.getCountries === core.getCountries`) — binding would break identity and introduce a destructuring divergence BIND-1 forbids. `surface.test.ts` (34 tests) guards the structure rather than any method name. Mutation evidence: hiding an override **1F**; binding forwarded members **2F**; dropping one core method **4F**. Ruling: Reviewer, topic `838-audit-vue`, following Solid's shipped architecture. |
...
\* The single CAT hit is `index.ts:217`, `Object.prototype.hasOwnProperty.call(overrides, prop)` — the
...
- **BIND-6 wrappers replaced by a reference-forwarding `Proxy` — `src/surface.test.ts`.** Ruled by the
  ...
  _surface drift_; implementing it revealed it was **already a live defect** — five core methods were
  unreachable through this binding and nothing had noticed. The proxy's one structural hazard (`#private`
  fields cannot be read through a proxy receiver, so they would force binding and lose identity) is pinned
  both structurally and behaviourally, with a positive control proving the structural regex matches real
  `#field` syntax. Mutations: hidden override 1F, bound members 2F, dropped method 4F.
