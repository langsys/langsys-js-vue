...
1. **`LangsysApp` is a `Proxy` over the base SDK's singleton, not a wrapper class.** It forwards every member **by reference** and overrides exactly two: `init` (widens `UserLocaleStore` to a `Signal<string>` and `writeGrant` to accept a `Ref`) and `setWriteGrant` (accepts a `Ref`). Forwarded members are returned **unbound**, so `LangsysApp.getCountries === coreLangsysApp.getCountries` — identity is the point, and binding would break it while introducing a behaviour difference BIND-1 forbids. This replaced a hand-written class that enumerated one delegating method per core method: that shape silently dropped every method the core added afterwards, and had already lost five (`applyAuthorization`, `getUserLanguagePreferences`, `parseAcceptLanguageHeader`, `findBestLocaleMatch`, `resolveLocale`) with a green suite. Apps that already own the locale in a `Ref` adapt it with `refToLocaleSource` (the Vue analog of the Svelte wrapper's `adaptStore`), which uses a `flush: 'sync'` watcher to preserve the Signal contract (synchronous notify, immediate first fire).
...
- **Never turn `LangsysApp` back into an enumerating wrapper, and never bind its forwarded members.** It is a `Proxy` forwarding by reference with exactly two overrides. Enumerating methods silently drops whatever the core adds next; binding breaks the identity that makes this binding excludable from an investigation. Both are pinned by `src/surface.test.ts`, which asserts every core method is reachable and identical rather than naming today's methods. The proxy is safe only while the core has no `#private` fields — that assumption is pinned too, structurally and behaviourally.
